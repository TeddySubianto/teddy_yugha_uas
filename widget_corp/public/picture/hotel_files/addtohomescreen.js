...
//Siteworx edit - remove android support
...
ath.Class.prototype = {
	...
	show: function (force) {
		...
		if ( this.options.message in ath.intl ) {		// you can force the locale
			...
			message = ath.intl[ath.language].message.replace('%action', ath.intl[ath.language].action[ath.OS]).replace('%device', ath.platform);
		}

		// add the action icon
		message = '<p>' + message.replace('%icon', '<span class="ath-action-icon">icon</span>') + '</p>';
		message = '<p>' + message.replace('%home-icon', '<span class="ath-home-icon">icon</span>') + '</p>';
